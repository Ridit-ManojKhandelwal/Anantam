...
const startServer = () => {
  ...
  const filePath = path.join(appPath);
  const serverPath = path.join(filePath, "server", "index.js");
  console.log(serverPath);

  const child = spawn("node", [serverPath]);

  // Handle the output of the spawned process
  child.stdout.on("data", (data) => {
    console.log(`Output: ${data}`);
  });

  // Handle any errors from the spawned process
